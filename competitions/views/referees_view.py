...
from ..models import Referees, Competitions
import json
from django.conf import settings
import requests
from requests.exceptions import Timeout

log = logging.getLogger('referees_log')



class RefereesView(BaseView):
    ...
    def get(self, request, competition_uuid, client, client_id):
        ...
        response = list(Referees.objects.filter(**params).values())
    ...
    def post(self, request, competition_uuid, client, client_id):
        ...
        member = Referees.objects.create(**data)
        response = member.pk
        # stat = BaseView.send_stat_log(request, client, client_id)
        # if not stat:
        BaseView.log(log, request, client, client_id)
...
class RefereeView(BaseView):

    @BaseView.authorization(('admin', 'anonymous'))
    def get(self, request, uuid, client, client_id):
        response = list(Referees.objects.filter(pk=uuid).values())
        if not response:
    ...
    def delete(self, request, uuid, client, client_id):
        entry = Referees.objects.filter(pk=uuid)
